<html>
<head>
    <link rel="stylesheet" href="https://stackpath.bootstrapcdn.com/bootstrap/4.3.1/css/bootstrap.min.css" integrity="sha384-ggOyR0iXCbMQv3Xipma34MD+dH/1fQ784/j6cY/iJTQUOhcWr7x9JvoRxT2MZw1T" crossorigin="anonymous">
    <script src="https://code.jquery.com/jquery-3.3.1.slim.min.js" integrity="sha384-q8i/X+965DzO0rT7abK41JStQIAqVgRVzpbzo5smXKp4YfRvH+8abtTE1Pi6jizo" crossorigin="anonymous"></script>
    <script src="https://cdnjs.cloudflare.com/ajax/libs/popper.js/1.14.7/umd/popper.min.js" integrity="sha384-UO2eT0CpHqdSJQ6hJty5KVphtPhzWj9WO1clHTMGa3JDZwrnQq4sF86dIHNDz0W1" crossorigin="anonymous"></script>
    <script src="https://stackpath.bootstrapcdn.com/bootstrap/4.3.1/js/bootstrap.min.js" integrity="sha384-JjSmVgyd0p3pXB1rRibZUAYoIIy6OrQ6VrjIEaFf/nJGzIxFDsf4x0xIM+B07jRM" crossorigin="anonymous"></script>
</head>
<body>

<?php
  session_start();
  if (isset($_SESSION['auth']) and $_SESSION['auth'] == true) {
      if (isset($_GET['exit'])) {
          session_destroy();
          header("Location: /blog/login.php");
      } else {
        header("Location: /blog/admin.php");
      }
  } else {
      if (isset($_POST['login']) and isset($_POST['password'])){
        if ($_POST['login'] == "Admin" and $_POST['password'] == "admin"){
            $_SESSION['auth'] = true;
            header("Location: /blog/login.php");
        } else {
            echo "<strong>Неверный пароль</strong>";
        }
      }
    echo "<h1 style='text-align: center;'>Вход</h2>
    <form method='post'>
      <div class='row'>
      <div class='offset-md-2 col-md-8 col-12'>
        <div class='form-group'>
            <label>Логин:</label> <input type='text' name='login' class='form-control'><br>
            <label>Пароль:</label> <input type='password' name='password' class='form-control'><br>
            <input type='submit' value='Вход' class='btn btn-primary'>
        </div>
      </div>
      </div
    </form>
    <a href='/blog/index.php' class='btn btn-primary'>На главную</a>";
  }
 ?>
</body>
</html>
